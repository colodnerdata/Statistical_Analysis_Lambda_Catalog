...
| v2.1 | Sequence axis + gap-aware longitudinal + serial-correlation diagnostics + Fixed Effects (Role axis, one-way only) | No | In progress — the Sequence/BFN chain, the FE Role dropdown + status-block validation, the CI+PI prediction layout, and the one-way FE engine (`Demean_By`/`y_s()`/`X_s_Within()`/`[DF_Absorbed]`/`Group_Prediction_Interval`, TODOs #2–#9) are all shipped; pending the QC-oracle update for the new prediction shape (`tools/inspect_regression_sheet.py` and friends) before the 2.1.0 Version History entry |
