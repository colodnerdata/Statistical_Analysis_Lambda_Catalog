...
# writes, replacing the bare 5/6/7/8 literals both used to carry. The v3.3
# block spans rows 3–9; the v3.4 CROSS-VALIDATED FIT sub-block sits at 10–13,
# so the v3.3 rows and the Comparison_Headline_GoF range ($AH$6:$AH$8) are
# untouched.
_ROW_UNIT_SMEARING = 5
_ROW_UNIT_R2 = 6
_ROW_UNIT_ADJ_R2 = 7
_ROW_UNIT_RMSE = 8
_ROW_UNIT_RESPONSE_SPACE = 9
_ROW_LOOCV_SUBHEADING = 10
_ROW_LOOCV_RMSE_UNIT = 11
_ROW_LOOCV_MAE_UNIT = 12
_ROW_SMEARING_TREATMENT = 13
...
_COLUMN_WIDTHS: tuple[tuple[int, float], ...] = (
    ...
    # bounds the row-2 header wrap (last content column = BB) and anchors the
...
_SMEARING_TREATMENT_NOTE = (
    ...
    "Smearing_Treatment plus a third item on the AH4 validation list, changed "
    "in lockstep with Unit_Space_LOOCV_Residual's smearing call."
)

# SE Regression (Unit) — AG8. Relabelled from "RMSE (Unit)" in v3.4 so the
