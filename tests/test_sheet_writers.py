...
def test_write_unit_space_block_writes_section_input_and_three_gof_cells() -> None:
    ...
    # plus the named treatment. Rows 3–9 are byte-identical to v3.3, so
    # Comparison_Headline_GoF ($AH$6:$AH$8) is untouched — the qualification is
    # scoped to the pair it caveats.
    assert sheet.cell(10, _C_AG).value == "CROSS-VALIDATED FIT"
    for row, label in [
        (11, "LOOCV RMSE (Unit)"),
        (12, "LOOCV MAE (Unit)"),
        (13, "Smearing Treatment"),
    ]:
        assert sheet.cell(row, _C_AG).value == label, row
    # The two numeric GoF cells take the same six-argument call as the rows
    # above, with the Method toggle from $AH$4.
    for row in (11, 12):
        formula = sheet.cell(row, _C_AH).api.Formula2
        assert formula is not None
        ...
        assert "Fit_Context()" in formula
        assert _A_BACK_TRANSFORM_METHOD in formula  # the $AH$4 anchor
    # AH13 holds the Smearing_Treatment text readout, which wraps (the full
    # explanation of what is approximated lives on the AG13 label's hover
    # Note, following the sheet's status-cell convention).
    treatment_formula = sheet.cell(13, _C_AH).api.Formula2
    ...
    assert sheet.cell(13, _C_AH).state.wrap_text is True
...
def test_write_residuals_appends_unit_space_columns_az_ba_bb() -> None:
    ...
    / Unit_Space_LOOCV_Residual with the AH4 Method input.
    """
    sheet = RecordingSheet(name="Regression")
    _write_residuals(_as_xw_sheet(sheet))
    ...
    assert sheet.cell(2, _C_BB).value == "LOOCV Residual (Original Units)"
    ...
    bb_formula = sheet.cell(3, _C_BB).api.Formula2
    assert az_formula == (
