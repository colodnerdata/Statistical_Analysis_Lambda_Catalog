...
**Beta — `_write_beta_fit`, a 2-D `Full_Factorial` dynamic-array spill** in a 6-column (`BY:CD`) zone, two stages side by side. Each stage's body is **one** spill at (row 33, the stage's first col): `=LET(…,grid,Full_Factorial($<Ncell>,VSTACK($<αmin>$,$<βmin>$),VSTACK($<αmax>$,$<βmax>$)),nll,BYROW(grid,LAMBDA(ab,NLL_Beta(z,INDEX(ab,1,1),INDEX(ab,1,2))+COUNT(d)*LN(scale_))),HSTACK(grid,nll))` — spills N²×3 (`Alpha | Beta | NLL`) across the stage's 3 cols. Stage 1 at BY33 (cols BY,BZ,CA); Stage 2 at CB33 (cols CB,CC,CD), refined bounds (α/β Min/Max = best±step from Stage 1). **No Data Table, no `Range.Table`, no `Grid_Search_Optimum`** — the artifact no longer uses any Excel Data Table. The control block is a vertical field-list with **two value columns per stage** (α and β side by side) so the block fits in ≤9 rows and clears the chart space at row 13:
