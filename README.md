...
- **Model Construction** — declarative model specification: each table column gets a Role (Response / Predictor / Identifier / Filter / Omit) and each predictor a Type (Continuous / Categorical, with reference-level control). The sheet derives the row mask, the constructed design matrix `X_s()`, and level-qualified column names.
