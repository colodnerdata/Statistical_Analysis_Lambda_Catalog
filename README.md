...
| [docs/MODEL_TESTING_ASSETS.md](docs/MODEL_TESTING_ASSETS.md) | The regression test-model suite: which model configurations the QC harness covers, which corner each one is there for, the datasets future milestones need, and the test-scale ordering the version ladder follows from v3.4 on | You're adding or changing a QC model case, wiring a new dataset, or wondering why the roadmap is sequenced the way it is |
