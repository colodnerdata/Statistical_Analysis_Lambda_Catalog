...
**Mark a case `heavy=True`** if its sheet is expensive — L07's ~2900 × 205 design matrix and L08's 173 Fixed Effects groups are the current two. The Python oracle always runs; only the sheet build is gated, behind `build_test_models.py --include-heavy`.
