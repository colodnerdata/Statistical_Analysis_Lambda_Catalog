...
1. **Regression work ships first.** Every milestone that extends the Regression sheet, its spec
   block, or its engine comes before any milestone that opens a *new* analysis surface. The
   three exceptions to the Regression track — Two-sample, Resampling, and the Time Series sheet —
   go last as a block, even though all three are flat-cost to test.
...
cases in the oracle that already exists, while Two-sample, Resampling and the Time Series sheet
each need a new sheet writer, a new layout, and a verification path sharing nothing with
...
| 2 | **`Cluster` role** | **v3.6** *(was unordered v3.9+)* | near-additive — a variance-estimator variant on a few models | Within-group correlated data: Production Lots facilities suffice initially (3 clusters — deliberately few, to test the small-cluster warning path); `Grunfeld` (item 5) later provides 10–11 proper clusters. |
| 3 | **`Time` role / lag-difference semantics** | **v3.7** *(was unordered v3.9+)* | near-additive — **and unlocks a today-gap** | A real **calendar-dated monthly series** (~144 rows, AirPassengers-shaped, with an actual date column). No wired dataset has dates; this asset also enables the Sequence **calendar-signature verdict** test in Section 1 immediately, before the Time role ships. The same series is later the whole of item 9's asset requirement, so wiring it here is what makes the Time Series sheet a no-new-data milestone. |
...
| R `Grunfeld` | 200 | balanced 10×20 two-factor panel; proper cluster count | **v3.9** two-way FE; back-fills **v3.6** `Cluster` |
