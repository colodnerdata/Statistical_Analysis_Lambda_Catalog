...
templates/
  static_sheets.xlsx         # pre-built copies of dataset-independent reference sheets
                              # (Regression Instructions, Modeling Concepts, Diagnostic
                              # Guide, Model Comparison Guide) — see "Static reference
                              # sheets" below
lambda_catalog/
...
`write_sheet_regression_instructions.py`, `write_sheet_modeling_concepts.py`, `write_sheet_diagnostic_guide.py` and `write_sheet_comparison_guide.py` — every module in the `_STATIC_SHEET_WRITERS` registry of `scripts/rebuild_static_sheets.py` — write sheets whose content never depends on the target dataset: a fixed how-to guide, a fixed modelling-concepts reference, a fixed diagnostics reference, and a fixed model-comparison reference. Rebuilding hundreds of styled cells with COM calls for unchanging text on every production build is wasted work, so these modules instead copy an already-styled sheet out of `templates/static_sheets.xlsx` via `workbook_helpers.copy_static_sheet` (Excel's native `Sheet.Copy` between two workbooks open in the same Excel instance — not an openpyxl round-trip; see AGENTS.md's "Use xlwings COM API for all chart creation — never openpyxl" for why openpyxl is unsafe for anything Excel-native like this). `write_regression_instructions_sheet(workbook)` / `write_modeling_concepts_sheet(workbook)` / `write_diagnostic_guide_sheet(workbook)` / `write_model_comparison_guide_sheet(workbook)` keep their original call signature, so the production build scripts and their tests are unaffected.

The authored content still lives in Python — `_ROWS` in `write_sheet_regression_instructions.py`, the body of `_write_template_sheet` in the others — but the production build scripts never execute it; they only call the copy-from-template functions above. Regenerating the template is a separate, manual step.

Run **`python scripts/rebuild_static_sheets.py`** after editing any of their content, then commit the updated `templates/static_sheets.xlsx` alongside the Python change. It opens the template once, calls every static sheet's `_write_template_sheet(workbook)` (so nothing is skipped or forgotten), and saves once. This is the standard command — prefer it over the per-module CLIs below, which exist only for regenerating a single sheet in isolation while debugging:
...
python -m lambda_catalog.write_sheet_modeling_concepts        # single-sheet debugging only
python -m lambda_catalog.write_sheet_diagnostic_guide         # single-sheet debugging only
python -m lambda_catalog.write_sheet_comparison_guide         # single-sheet debugging only
```
...
**The staleness itself is checked without Excel**, which is the asymmetry worth remembering: regeneration needs a desktop, detecting a forgotten regeneration does not. `tests/test_static_template_freshness.py` parses every registered writer's source with `ast` and reads `templates/static_sheets.xlsx` as a zipfile, asserting that every string literal a writer puts in a cell appears in that sheet — so a stale template fails the Linux CI job instead of shipping. It is one-directional (extra or hand-edited content in the template is invisible to it) and cannot see layout; the Excel-machine path remains the only check for those. Three things about its extraction are worth knowing before editing it: the cell text reaches the sheet through three write paths only (a pinned content constant, a `_heading`/`_subheading`/`_table_header_row`/`_row` call, or an `x.value =` assignment); a cell built by f-string interpolation is only checkable in its static segments — the interpolated value itself is not statically knowable and is out of scope by design; and the reader must handle both storage forms, because Excel picks inline strings (`t="inlineStr"`) or a shared-string table (`t="s"` → `xl/sharedStrings.xml`) by a size heuristic the writer does not control — the template workbook and the built `dist/` artifact are stored in different forms, which is exactly how a reader that knew only one form silently reported every literal as stale.

**A sibling check guards the addresses those sheets cite**, in `tests/test_static_sheet_citations.py`: the static sheets name sheet addresses as literal text ("Col AT, Residual Output", "Cell AH5"), nothing derives them, and the Diagnostic Guide's citations had already drifted twice. It pins each cited address to the `regression_layout` / `spec_layout` constant that names it and fails on a citation no pin covers, a pin no writer cites, or a citation whose letter no longer matches the layout — pure AST plus the imported constants, so it runs in the same Excel-free CI job. Only *literal* citations are checkable; an address built by f-string interpolation from a layout constant cannot drift and is out of its scope by design.
...
**Two further checks in the same family guard an artifact rather than the docs.** `tests/test_static_template_freshness.py` compares the static-sheet writers' source against the committed `templates/static_sheets.xlsx`, and `tests/test_static_sheet_citations.py` compares the sheet addresses those writers cite against the layout constants that name them — both pure Python and Excel-free, so they run in the same CI job. Their subject is the template drifting from its writers, and a citation drifting from the layout, rather than docs drifting from code; both are documented in *Static reference sheets* above.
