...
**Why the split stays even though the Data Tables are gone.** Both artifacts now run plain **Automatic** — the Univariate sheet no longer uses any Excel Data Table (Beta's two-stage grid search is a `Full_Factorial` → `BYROW` NLL → `HSTACK` dynamic-array spill sized by an in-sheet N cell), so the original driver for the split ("Automatic except Data Tables" was the only mode that could ship a workbook containing a Data Table) no longer applies. The split remains for two reasons that still hold. First, **recalc cost is asymmetric and the fast-iteration flags differ**: the Regression workbook's `CalculateFullRebuild` is cheap (no Data Tables, no big spills), while the Univariate workbook's Beta `Full_Factorial` spills make a full recalc the slow step of that build, so `build_univariate.py` keeps `--skip-data-table-calculations` (flag name retained for CLI stability; it now skips the Beta grid-spill rebuild, not a Data Table) and `--no-calculation` as its fast-iteration knobs, which `build_production.py` does not need. Second, **artifact size and audience**: each workbook carries its own data sheets, sheets, charts, and version lineage, and the Regression modeler and the Univariate distribution-fitter are different users who do not want the other's sheets. A follow-up `docs/DECISIONS.md` entry records that the original Data-Table rationale was retired when Beta moved to `Full_Factorial` and that the split was kept on these grounds rather than re-merged. See [DECISIONS.md § v3.0](docs/DECISIONS.md#univariate-becomes-its-own-workbook).

**Two named build targets, not flags.** `build_production.py` (Regression) and `build_univariate.py` (Univariate) are separate driver scripts sharing one scaffolding module (`lambda_catalog/build_common.py`). The old `--skip-univariate` flag is retired — the Regression target simply never writes the Univariate sheet, and the Univariate target never writes the Regression sheets. `--skip-data-table-calculations` survives (flag name kept for CLI stability) and is the Univariate target's primary fast-iteration flag (it skips the slow Beta `Full_Factorial` grid-spill rebuild there — there is no longer any Data Table to skip).
