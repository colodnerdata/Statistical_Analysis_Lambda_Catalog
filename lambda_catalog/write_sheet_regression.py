...
def _write_residual_conditional_formatting(sheet: xw.Sheet) -> None:
    ...
    # Scaled against the LOOCV RMSE (Unit) cell at $AH$11 (built from the row
...
def _write_model_specification(sheet: xw.Sheet) -> None:
    ...
    # SE Regression (Unit) (AG8) and Smearing Treatment (AG13). v3.4 relabelled
    # AG8 from "RMSE (Unit)" and added the LOOCV sub-block; both notes are
    ...
    _set_note(sheet, 8, _C_AG, _SE_REGRESSION_UNIT_NOTE, label="SE Regression (Unit)")
    _set_note(sheet, 13, _C_AG, _SMEARING_TREATMENT_NOTE, label="Smearing Treatment")
...
def _write_unit_space_block(sheet: xw.Sheet) -> None:
    """v3.3 unit-space / back-transformation block at AG3:AH9, plus the v3.4
    CROSS-VALIDATED FIT sub-block at AG10:AH13.
    ...
    the active state visible at a glance. The v3.4 sub-block (rows 10–13)
    leaves rows 3–9 and the Comparison_Headline_GoF range ($AH$6:$AH$8)
    ...
    | Row | AG                     | AH                                         |
    |-----|------------------------|--------------------------------------------|
    | 3   | "UNIT-SPACE FIT"       | (section heading)                         |
    ...
    | 8   | "SE Regression (Unit)" | =Unit_Space_RMSE(...)                      |
    | 9   | "Response Space"       | readout (Fit_Context → "Log"/"None")       |
    | 10  | "CROSS-VALIDATED FIT"  | (section heading)                         |
    | 11  | "LOOCV RMSE (Unit)"    | =Unit_Space_LOOCV_RMSE(...)                |
    | 12  | "LOOCV MAE (Unit)"     | =Unit_Space_LOOCV_MAE(...)                 |
    | 13  | "Smearing Treatment"   | =Smearing_Treatment(...) (WrapText)        |
    """
    ...
    # ── v3.4 CROSS-VALIDATED FIT sub-block (AG10:AH13) ─────────────────────
    # Sits below the v3.3 block so rows 3–9 and the Comparison_Headline_GoF
    # range ($AH$6:$AH$8) stay byte-identical. The two out-of-sample error
    ...
    # with $AH$4 built from _A_BACK_TRANSFORM_METHOD, never spelled.
    section_heading(sheet, 10, _C_AG, "CROSS-VALIDATED FIT")
    for row, label, formula in [
        (
            11,
            "LOOCV RMSE (Unit)",
        ...
        f(sheet, row, _C_AH, formula)
        sheet.range(rc(row, _C_AH), rc(row, _C_AH)).number_format = "0.0000"
    val(sheet, 13, _C_AG, "Smearing Treatment")
    f(
        sheet,
        13,
        _C_AH,
    ...
    sheet.range(rc(13, _C_AH), rc(13, _C_AH)).api.WrapText = True
...
def _write_residuals(sheet: xw.Sheet) -> None:
    ...
    # v3.4 unit-space LOOCV column. Same opt-out as AZ/BA: it is in original
    # units by construction, so the conditional (Log)/(Within …) suffix logic
    # above does not apply to it.
    val(sheet, 2, _C_BB, "LOOCV Residual (Original Units)")
    bold_row(sheet, 2, _C_AN, _C_BB)
    ...
    # BB: LOOCV residual in original units — the leave-one-out sibling of BA.
    # Observed response in original units (Unit_Space_Observed, never smeared)
    # minus the back-transform of the LOO fitted value. Carries the Method
    # toggle from AH4 so flipping Duan → Naive propagates here too. A row with
    # leverage h_i = 1 yields #N/A (the IFERROR inside the LAMBDA), matching
    # the PRESS Residual column's own #N/A on such rows.
    f(
        sheet, 3, _C_BB,
        (
...
def write_regression_output_sheet(
    ...
    sheet.range(rc(2, _C_S), rc(2, _C_BB)).api.WrapText = True
