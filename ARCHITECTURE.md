...
|---|---|---|---|
| **Variable Role** | `Response (y)` · `Predictor (x)` · `Identifier (Row Label)` · `Filter` · `Omit` · `Fixed Effects` (v2.1) | Cluster (v3.7), Time (v3.8), Weight (v3.9) | What the column *is* in the model |
...
- **At most one** of each Role value that is single-instance by nature:
  ...
  rows), Cluster, Time, Weight (all still future — v3.7/v3.8/v3.9). The status block
...
- **Fixed Effects** *(v2.1)* — the panel role: enters no column; the entire
  ...
  `Absorb_Two_Way_Fixed_Effects` is its own post-v2.1 milestone, see v3.10).
- **Cluster** *(v3.7)* — clustered-robust variance estimator. Forward
...
- **Weight** *(v3.9)* — WLS. See DECISIONS § v2.6, recorded under the
...
as the v3.3 remainder, and now ships **last on the ladder, at
[v3.11](ROADMAP.md#v311--standalone-data-transformation-library--planned)** —
it is the only item that widens the predictor-transform axis every model is
crossed against, so it is the most expensive thing in the plan to test (see
...
[v3.10](ROADMAP.md#v310--two-way-fixed-effects--planned)**.
...
`lambda_functions.json`. They are v3.11 work items in
[TODOs.md](TODOs.md#v311--standalone-data-transformation-library) (planned as
...
- A `SWITCH` argument that is a Role-axis value can carry a dormant branch
  ...
  change. The `Cluster` work (v3.7) lights up the dormant branch by adding
