...
# second case, on the scale the statistics are actually reported in.
# ``compare_values`` floors the divisor at 1.0, so a small response gets no
# adjustment and the comparison is never made stricter than the absolute scale.
# Applying it to the existing ``Unit_Space_RMSE`` removes a latent
# over-strictness (it was previously on the absolute scale, where a response
# in the billions demands six-decimal agreement the IEEE-754 floor will not
# give).
...
def read_case_comparison_rows(
    ...
    # divide by a response-derived scale too — the same response-unit precision
    # floor the residual band carries, applied to single cells instead of n
    # rows.
    dependent = tuple(
    ...
    # ── v3.3/v3.4 unit-space block (AH6:AH13) ────────────────────────────
    # Compared cell-for-cell here: the Back-Transform toggle (L02 vs L03)
    # is only meaningful if the in-sample quartet (rows 6–9) is read, and the
    # v3.4 out-of-sample pair (rows 12–13) is what the toggle's smearing
    ...
    # a response-derived scale too); see there for the divisor rationale.
    residual_rows = [
        _row(
    ...
    # ── v3.4 original-units residual band (AZ–BB, n rows from row 4) ──────
...
def read_smearing_treatment(sheet: xw.Sheet) -> object:
    """The v3.4 ``Smearing Treatment`` readout at ``AH14``, as written text.
