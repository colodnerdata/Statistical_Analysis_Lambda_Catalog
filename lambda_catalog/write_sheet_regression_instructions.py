...
_ROWS: list[tuple[int, str, str | None]] = [
    ...
    (
        ...
        (
            ...
            "Product, Difference, or Ratio. The interaction column is built "
            "into the design matrix automatically: Continuous × Continuous "
            "adds 1 column, Continuous × Categorical adds one per retained "
            "level, and Categorical × Categorical adds the full product of "
            "the two. Naming this row's own variable with Operation = "
            "Product is the documented way to declare a quadratic (x²) "
            "term. An interaction whose mate is switched off (Include = "
            "FALSE) still builds and is flagged amber as a marginality "
            "warning; a reciprocal declaration — two rows naming each other "
            "under Product or Difference — is flagged red, because it would "
            "duplicate or negate a column and make the matrix singular. "
            "Interaction rows also appear in PREDICTION INPUTS; leave them "
