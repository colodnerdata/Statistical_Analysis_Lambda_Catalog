...
Row 1 holds the top-level zone labels ("MODEL SPECIFICATION", "PREDICTOR SUMMARY", "REGRESSION OUTPUTS", "PREDICTION OUTPUTS", "RESIDUAL OUTPUT"). Lower section headings appear at the relevant data rows within each zone. The MODEL SPECIFICATION zone (A–L) is the shared spec block imported from `write_sheet_model_construction.py` (headers row 3, spec rows 4–26, Intercept control A2/C2, Sequence status line H2; H = Sequence structural flag, I = Sequence Period (pure override input), J = Period In Use (computed display: I's override when present, else the computed candidate), K/L = Levels / Reference In Use displays; a jump-link at row 27 and the Sequence Spacing block relocated past the end of the 16000-row Spec_* bands at rows 16002–16008 — Δ candidate/in-use, verdict lines, delta spectrum at K/L — so a live retargeted Source_Table wider than the shipped sample can never spill into it); every other zone keeps headers on row 2 with spills from row 3. Every `_C_*` column constant in `write_sheet_regression.py` matches its actual column letter (`_C_N` is column N).
