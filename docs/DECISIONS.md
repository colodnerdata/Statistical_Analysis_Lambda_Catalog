...
**Resolution:** yes — the split stays, but on different grounds. Beta's grid search is now a single `Full_Factorial(N, mins, maxs)` → `BYROW` NLL → `HSTACK` spill of N²×3 per stage (two stages side by side in a 6-column zone BY:CD, N editable live in the sheet; default N=10), so **the Univariate workbook no longer uses any Excel Data Table** and both artifacts run full Automatic. The original driver — "Automatic except Data Tables" is the only mode that can ship a workbook containing a Data Table — is retired. The split remains for two reasons that still hold: (1) the recalc cost is asymmetric — the Regression workbook's `CalculateFullRebuild` is cheap, while the Univariate workbook's Beta `Full_Factorial` spills make a full recalc the slow step of that build, so `build_univariate.py` keeps `--skip-data-table-calculations` (flag name retained for CLI stability; it now skips the Beta grid-spill rebuild, not a Data Table) and `--no-calculation` as its fast-iteration knobs; and (2) the two artifacts are genuinely different products — different data sheets, analysis sheets, charts, version lineages, and audiences (a Regression modeler vs. a Univariate distribution fitter). Do not re-merge.

**Supersedes** the v3.0 *Univariate becomes its own workbook* entry's Data-Table rationale paragraph (the split decision itself is upheld — only the stated *reason* changes) and closes the "Beta's method-of-moments start and ~12×12 grid are still open" tail of the *grid shrink* entry. The CLAUDE.md / CONTRIBUTING.md / README.md narratives were re-grounded on build/calc-mode isolation + artifact size, not re-merged.

**Breakage class: MAJOR for the Univariate workbook version only.** The three fit zones relayout from 9/9/21 cols (stacked stages, Data Table) to 4/4/6 cols (stages side by side, vertical field-list control block, profile-NLL charts re-anchored above the body at rows 13–30, body rows 33+). The Beta Scale/Shape Min/Max/Step input cells the old Data Table exposed are replaced by α/β Min/Max/Step cells feeding `Full_Factorial`'s `VSTACK(mins)/VSTACK(maxs)`, and the old `UV_BETA_S1`/`UV_BETA_S2` ranges are replaced by `UV_BETA_S{1,2}_{Alpha,Beta,NLL}` OFFSET ranges sized by the live N cell. A user's saved Beta bounds carry over in spirit but not in cell address. No Regression-side change.
