...
_ROWS: list[tuple[int, str, str | None]] = [
    ...
    (
        ...
        (
            ...
            "Product, Difference, or Ratio. The constructor builds the "
            "interaction columns immediately after the declaring row's own "
            "block, so the design matrix stays in specification order: one "
            "column for Continuous × Continuous, one per non-reference level "
            "for Continuous × Categorical, and one per level pair for "
            "Categorical × Categorical. Headers join the two operands' "
            "names with the operation's own symbol (e.g. \"Weight × Origin: "
            "US\", \"Weight ÷ Displacement\"), so a header says what was done "
            "as well as what it was done to. An operand that is an excluded "
            "Predictor still builds — flagged amber, because an interaction "
            "without its main effect is a marginality violation that is "
            "usually a mistake but occasionally deliberate. An operand that "
            "is not a Predictor or matches no column contributes nothing "
            "and stays flagged red — the constructor degrades to the main "
            "effect rather than taking the sheet down for one mistyped "
            "cell, the same way an invalid Reference Level does. A "
            "reciprocal declaration (this row names the other, the other "
            "names this one back, both under Product or Difference) is "
            "flagged red — a duplicate column would make the Gram matrix "
            "singular. Ratio is exempt (B/A is a different column from "
            "A/B), and a row naming ITSELF is exempt: self × self under "
            "Product is the documented quadratic term. Ratio returns #N/A "
            "where the denominator is zero, rather than a #DIV/0! error. "
            "Two-way interactions only. One thing is deliberately not "
            "automatic: an interaction row in the Prediction Inputs band "
            "is an independent input, not recomputed from its operand "
            "rows — the band's header note says so rather than silently "
            "rewriting one user input because another changed. "
            "The Design Columns column (O) shows how "
