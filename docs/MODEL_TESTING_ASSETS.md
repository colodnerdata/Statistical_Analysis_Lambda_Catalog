...
| L3 | L2 with Back-Transform = **Naive** | flips `AH5` | naive point estimate `EXP(ŷ)`; confirms CI/PI bounds are EXP-only under both settings. **Required an oracle change** — the Python side computed both branches and discarded the Naive one, so the back-transform toggle had never been verified against anything | existing — `life_log_response_naive` |
...
**`--verbose` names each sheet before writing it**, not after. A ~50-sheet run
