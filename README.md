...
The workbook's **Version History** sheet is the record — one row per release:

```
3.4.0   2026-09-20   No   Model Comparison — the cross-sheet aggregation layer ...
```
...
One workbook, built by one script (`build_production.py`). It carries the full function library — all 155 LAMBDA definitions (130 workbook-scoped, plus 25 sheet-scoped template constructors) — plus the pre-built templates and the reference and data sheets. Twelve sheets, in tab order: Regression, Regression Instructions, Modeling Concepts, Diagnostic Guide, Model Comparison, Model Comparison Guide, Univariate, LAMBDA_functions, Version History, Production Lots, Life Expectancy Data, Mileage Data. The tab-by-tab inventory is on the workbook tour at [Documentation map](#documentation-map).
...
- **Modeling Concepts** — per-feature explainers (Fixed Effects, reference levels, Sequence, Log transforms, interactions, sample filtering, intercept control): the point of each feature, the statistical method it enables, and a use case. A reference sheet.
...
- **Model Comparison Guide** — choosing between candidate specifications: when a comparison is valid at all (same response, same transform, same sample, same fixed-effects structure, same back-transform method), what each statistic is for and whether it survives a transform change, a numbered procedure, the traps, and what the workbook deliberately does not provide. A reference sheet.
- **Model Comparison** — the cross-sheet registry the guide explains, and a pre-built template. One row per registered model, read out of the finished Regression sheets: a registry block (the anchor, the derived sheet name and hyperlink, the assembled formula string), the goodness-of-fit columns **split by comparability** — a unit-space block (R², adjusted R², RMSE in original units) that survives a response-transform change and a fit-space block (F, R², AIC/BIC/AICc, PRESS …) that does not — and a read-only prediction comparison. Four per-row **gates** (same set? same space? same back-transform method? same inputs?) flag the rows a given block may not be compared across, and a row-2 verdict says how many registered rows share each one. Adding a model is one edit in Name Manager: point that row's `Comp_Anchor_<n>` at the target sheet's response readout.
