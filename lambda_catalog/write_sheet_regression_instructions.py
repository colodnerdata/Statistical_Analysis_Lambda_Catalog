...
_ROWS: list[tuple[int, str, str | None]] = [
    ...
    (
        ...
        (
            (
                "Open the Name Manager (Formulas → Name Manager) and update Source_Table (the \"Refers"
                " To\" field) to your table, including its header row — for example =MyTable[#All]. "
                "Everything else derives from this one name: the header row, the data body, and the "
                "variable list in the MODEL SPECIFICATION block all update automatically. "
                "Source_Table points at LifeExpectancyData by default (a curated four-driver Life "
                "Expectancy model — Adult Mortality, Alcohol, percentage expenditure, and Status); a "
                "second sample table, MileageData (on the Mileage Data sheet), ships with the "
                "workbook for a multi-level categorical-encoding demo (MPG ~ Horsepower + Weight + "
                "C(Model Year) + C(Origin)). You can use a supplied table or your own Excel Table. "
                "After changing the source, review every Role, Include setting, and Type; existing "
                "inputs are not inferred from the new headers."
            )
        ),
        "body",
    ),
    ...
    (
        ...
        (
            (
                "Review Role for every source column, and Include and Type for each Predictor; fill "
                "these in for any additional specification rows — the dropdowns are available all the"
                " way down. A row with a blank Role is ignored. The Order column is a placeholder for"
                " a future version and is not read by any formula; it is hidden on the sheet.\n"
                "\n"
                "The Transform column offers Log and Log (drop ≤ 0) on the Response row and on "
                "Continuous Predictor rows: the model fits in natural-log space for that variable, "
                "and affected outputs are labeled \"(Log)\" — but the Unit-Space Fit block at AG4:AH10 "
                "reports the back-transformed R² / Adj R² / RMSE in original units (Duan smearing by "
                "default, Naive on the AH5 toggle), and the Prediction Outputs block's Original Units"
                " column (AL) carries the back-transformed point estimate and the four CI/PI bounds. "
                "The two new Residual Output columns (AZ, BA) carry Predicted Y and Residual in "
                "original units. With a Log response, interval bounds always use Naive back-"
                "transformation, regardless of the toggle. They are not confidence bounds for the "
                "Duan-adjusted mean. Log is not valid on a Categorical Predictor; setting it there "
                "flags the cell red and the fit still runs with that row's Transform ignored (dummy-"
                "coded as usual). Plain Log leaves nonpositive values in the sample and returns #N/A;"
                " Log (drop ≤ 0) excludes them and reports the count.\n"
                "\n"
                "The Sequence column marks at most one variable as the ordering axis used by "
                "lag/difference functions and serial-correlation diagnostics — it is independent of "
                "Role and Type (a Predictor can also be the sequence axis). Leave it blank for non-"
                "panel data; marking two or more rows shows a red error in the status cell above the "
                "column. The Sequence Period column (I) is the typed override input: type a number on"
                " the flagged row to declare a Δ that differs from the candidate. The Period In Use "
                "column (J) is the live companion — it shows the typed override if column I is non-"
                "blank, otherwise the computed candidate (the most common gap between consecutive "
                "periods within a group). Lag_By and Difference_By fall back to Base_Period_Delta() "
                "when their [delta] argument is omitted.\n"
                "\n"
                "The Interaction Term (M) and Interaction Operation (N) columns declare an "
                "interaction between this row and another Predictor — pick the other variable and one"
                " of Product, Difference, or Ratio. The interaction column is built into the design "
                "matrix automatically: Continuous × Continuous adds 1 column, Continuous × "
                "Categorical adds one per retained level, and Categorical × Categorical adds the full"
                " product of the two. Naming this row's own variable with Operation = Product is the "
                "documented way to declare a quadratic (x²) term. An interaction whose mate is "
                "switched off (Include = FALSE) still builds and is flagged amber as a marginality "
                "warning; a reciprocal declaration — two rows naming each other under Product or "
                "Difference — is flagged red, because it would duplicate or negate a column and make "
                "the matrix singular. Interaction rows also appear in PREDICTION INPUTS; leave them "
                "at the Training Mean defaults or type scenario values.\n"
                "\n"
                "The Design Columns column (O) shows how many design-matrix columns each row "
                "contributes — 1 for a Continuous predictor, one less than the Levels count for a "
                "Categorical one. The total above it, with the intercept added, is the full width of "
                "the constructed design matrix; it turns amber on a model wide enough to be slow and "
                "red on one too wide for the sheet."
            )
        ),
        "body",
    ),
    ...
    (
        ...
        (
            ...
            "numeric, and every Filter column is truthy. Include settings and "
            "Log (drop ≤ 0) also affect the sample. Categorical predictors impose no "
    ...
    (
        ...
        (
            ...
            "(AB13) in REGRESSION OUTPUTS. If a Fixed Effects variable is declared, "
