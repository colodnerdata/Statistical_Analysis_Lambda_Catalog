...
templates/
  static_sheets.xlsx         # pre-built copies of dataset-independent reference sheets
                              ...
                              # Guide) — see "Static reference sheets" below
lambda_catalog/
...
`write_sheet_regression_instructions.py`, `write_sheet_modeling_concepts.py` and `write_sheet_diagnostic_guide.py` write sheets whose content never depends on the target dataset — a fixed how-to guide, a fixed modelling-concepts reference, and a fixed diagnostics reference. Rebuilding hundreds of styled cells with COM calls for unchanging text on every production build is wasted work, so these three modules instead copy an already-styled sheet out of `templates/static_sheets.xlsx` via `workbook_helpers.copy_static_sheet` (Excel's native `Sheet.Copy` between two workbooks open in the same Excel instance — not an openpyxl round-trip; see AGENTS.md's "Use xlwings COM API for all chart creation — never openpyxl" for why openpyxl is unsafe for anything Excel-native like this). `write_regression_instructions_sheet(workbook)` / `write_modeling_concepts_sheet(workbook)` / `write_diagnostic_guide_sheet(workbook)` keep their original call signature, so the production build scripts and their tests are unaffected.

The authored content still lives in Python — `_ROWS` in `write_sheet_regression_instructions.py`, the body of `_write_template_sheet` in the other two — but the production build scripts never execute it; they only call the copy-from-template functions above. Regenerating the template is a separate, manual step.
...
**The staleness itself is checked without Excel**, which is the asymmetry worth remembering: regeneration needs a desktop, detecting a forgotten regeneration does not. `tests/test_static_template_freshness.py` parses the three writers' source with `ast` and reads `templates/static_sheets.xlsx` as a zipfile, asserting that every string literal a writer puts in a cell appears in that sheet — so a stale template fails the Linux CI job instead of shipping. It is one-directional (extra or hand-edited content in the template is invisible to it) and cannot see layout; the Excel-machine path remains the only check for those. Two things about its extraction are worth knowing before editing it: the cell text reaches the sheet through three write paths only (a pinned content constant, a `_heading`/`_subheading`/`_table_header_row`/`_row` call, or an `x.value =` assignment), and a cell built by f-string interpolation is only checkable in its static segments — the interpolated value itself is not statically knowable and is out of scope by design.
...
**A fourth check in the same family guards a different artifact.** `tests/test_static_template_freshness.py` compares the static-sheet writers' source against the committed `templates/static_sheets.xlsx` — also pure Python and Excel-free, so it runs in the same CI job. Its subject is the template drifting from its writers rather than docs drifting from code; it is documented in *Static reference sheets* above.
