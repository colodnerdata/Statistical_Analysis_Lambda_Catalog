...
**Content-column widths are keyed on the `_C_*` constants too — `_COLUMN_WIDTHS`.** That table was a dict of literal column LETTERS until the four zones after the spec block were found sized for the pre-layout-break grid: the v3.0 break shifted every zone right by three columns and left the width keys behind, so the Predictor Summary's name column (S) got a stats column's 9, the Regression Outputs' diagnostics labels (AD) got the width meant for its values, and the whole Prediction Outputs zone (AJ–AL) fell off the table and rendered at Excel's default 8.43. Nothing failed — the build sized a different column, which is the same silent-wrong-answer mode the "never spell an A1 address into a formula" rule exists to stop. `_COLUMN_WIDTHS` is now `(constant, width)` pairs with a module-level assertion that every zone content column is sized exactly once and no gap column is sized at all, so the next shift fails at import instead of shipping. The spec block (A–O) keeps its own widths in `write_sheet_model_construction._SPEC_COLUMN_WIDTHS`; column I (the Regression-only Verdict overlay) and BB (the post-zone chart gutter) are the two deliberate entries outside the zones.

The v3.3 shift that took Residual Output from `AN–AY` to `AN–BA` is a content change, not a layout break: the chart anchor moved from `_C_AZ` to `_C_BB` (the two new columns were inserted *before* it, so the chart anchor and the entire §4b materialization band derive right by 2 and BP at column 68 is preserved). The **Model Formula readout** is no longer in this zone: it shipped at `AA2:AB2`, at the head of Regression Outputs, where — because row 2 wraps and then AutoFits — the sheet's longest string set the height of the entire header row. It now sits on **row 1 of the terminal Constructed Design Matrix zone**, right of that zone's own heading — header at `_C_DESIGN_MATRIX + 2`, the readout three columns past it, both derived from the zone anchor — with WrapText explicitly FALSE. Row 1 is the one row in that zone the matrix itself can never reach (its names spill on row 2 and its values on row 3, and both grow *rightward*), so the caption is never displaced by an ordinary modelling choice and the §4b ordering rule is not in play: nothing is placed to the RIGHT of the zone, the caption is placed ABOVE its body inside the zone's own columns. With wrap off and nothing else on row 1, the string overflows across as many empty columns as it needs; the three-column header/readout gap is what keeps the header itself from being clipped. Note the consequence: that zone ships COLLAPSED, so the caption is hidden until the zone is expanded. The cell holds `=Model_Formula()` — the sheet-scoped catalog closure that assembles `<response> ~ 1 + <predictors> [| <FE>]` from the spec — not an inline expression. The sheet-scoped `Comparison_Anchor`, `Comparison_Headline_GoF`, and `Comparison_Model_Formula` named ranges are the v3.4 Model Comparison sheet's reading surface — registered in `_setup_local_names` next to the `RegChart*` entries, and `Comparison_Model_Formula` is precisely why moving the readout cost its consumer nothing: v3.4 reads the NAME, never the address.
